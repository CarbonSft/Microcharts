...
namespace Microcharts
{
    ...
    /// 
    /// Radial gauge chart. Supports only one series of data so far.
    /// </summary>
    public class RadialGaugeChart : Chart
    {
        ...
        private float AbsoluteMinimum => Series.First().Select(x => x.Value)
            .Concat(new[] {MaxValue, MinValue, InternalMinValue ?? 0}).Min(x => Math.Abs(x));

        private float AbsoluteMaximum => Series.First().Select(x => x.Value)
            .Concat(new[] {MaxValue, MinValue, InternalMinValue ?? 0}).Max(x => Math.Abs(x));
        ...
        public override void DrawContent(SKCanvas canvas, int width, int height)
        {
            ...
            var cx = width / 2;
            var cy = height / 2;
            var lineWidth = (LineSize < 0) ? (radius / ((Series.Count() + 1) * 2)) : LineSize;
        ...
        private void DrawCaption(SKCanvas canvas, int width, int height)
        {
            ...
            foreach (var entry in Series.First())
            {
